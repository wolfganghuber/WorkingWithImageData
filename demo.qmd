...
% I followed the top-voted reply there, and ran 
...
ffmpeg -ss 00:03:34 -t 00:03:45 -i eakKfY5aHmY.mp4 frames/murm-%03d.png
```
...
#| cache: FALSE
library("EBImage")
frames = dir("fig/frames/", full.names = TRUE) |> sort()
frames = frames[1:100]
mov = readImage(frames)
movg = mov[,,1,] + mov[,,2,] + mov[,,3,]
colorMode(movg) = "grayscale"
nframes = dim(movg)[3]
ddtmovg = movg[,,2:nframes] - movg[,,1:(nframes-1)] 
```
...
res = array(NA_real_, dim = c(2, length(cx), length(cy), t2-t1))
time = t1:(t2-1)
for(it in seq(along=time)) {
  im1 = movg[,,time[it]]
  im2 = movg[,,time[it]+1]
  for(ix in seq(along = cx)) {
    for(iy in seq(along = cy)) {
      ...
      f2 = stats::fft(win2)
      d = solve(f1, f2)
      #res[, ix, iy, it] = c(abs(d[1,2]), abs(d[2,1]))
      res[, ix, iy, it] = c(1,1) * abs(d[1,1])
    }
  }
}

for(it in seq(along=time)) {
display(normalize(movg[,,time[it]]), method="raster")
v = res[1,,,it] + res[2,,,it]
v = 1 + round(100*(v/max(v)))
eg =  expand.grid(cx, cy)
points(x = eg[,1], y = eg[,2], pch=16, col = colorRampPalette(brewer.pal(9, "Blues"))(101)[v])
}
```
