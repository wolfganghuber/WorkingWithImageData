...
{{< video fig/murmuration-flow.mp4 >}}
...
rsync -avz /Users/whuber/svnco/WorkingWithImageData/fig/murmuration-flow.mp4 whuber@datatransfer.embl.de:/g/huber/www-huber/users/whuber/2304-Imaging-Data-in-R/resources/murmuration-flow.mp4
...
for(it in seq_len(length(time) - 1)) {
  ...
  for(ix in seq(along = cx)) {
    ...
    for(iy in seq(along = cy)) {
      ...
      # to do: instead of max.shifts, could regularize towards (0,0) if the difference between the other value and (0,0) is not 'significant'
...
#| eval: FALSE
ffmpeg -framerate 12 -pattern_type glob -i 'fig/opticalflow/*.png' -c:v libx264 -pix_fmt yuv420p fig/murmuration-flow.mp4 
```
